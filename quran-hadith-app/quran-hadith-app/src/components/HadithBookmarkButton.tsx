...
export default function HadithBookmarkButton({
  hadithId,
  variant = 'ghost',
  size = 'icon',
  className = '',
  ...
  useEffect(() => {
    const checkBookmark = async () => {
      if (status === 'authenticated' && session?.user) {
        try {
          const response = await fetch('/api/bookmarks/hadith');
          if (response.ok) {
            const data = await response.json();
            const bookmark = data.bookmarks.find(
              (b: any) => b.hadithId === hadithId
            );
  ...
  const handleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        // Remove bookmark
        const response = await fetch(`/api/bookmarks/hadith?hadithId=${hadithId}`, {
          method: 'DELETE',
        });
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          disabled={loading}
          title={isBookmarked ? 'Edit bookmark' : 'Bookmark this hadith'}
        >
          {loading ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : isBookmarked ? (
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {isBookmarked
              ...
              : 'Add a personal note to this hadith (optional)'}
          </DialogDescription>
        </DialogHeader>
